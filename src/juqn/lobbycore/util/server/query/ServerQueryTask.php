...
final class ServerQueryTask extends AsyncTask {
	...
	public function onRun() : void {
		...
		foreach ($this->servers as $name => $server) {
			...
			if ($result !== null) $data[$name] = $result;
		}
		$this->setResult($data);
	}
	...
	public function onCompletion() : void {
		...
		if ($result !== null) {
			...
			foreach ($result as $serverName => $serverData) {
				...
				if ($serverData['result'] === 'error') $result = $server->update(ServerState::OFFLINE());
				else {
				...
				if (!$needUpdate && $result) $needUpdate = true;
			}

			if ($needUpdate) ServerMenu::getInstance()->update();
		}
	}
}